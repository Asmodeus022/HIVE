...
function register($username, $companyname, $location, $businesscategory, $businesssize, $email, $pword, $conn) {
    if (checkIfEmailExists($email, $conn)) {
        ...
        <strong>Existing user,</strong> Your preferred email is not available!
        ...
        // Insert the owner data
        $otp = '0';
        $hashpass = md5($pword);
        $query = "INSERT INTO owners (Username, company_name, location, category, business_size, email, password, otp, email_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, 'ssssssssi', $username, $companyname, $location, $businesscategory, $businesssize, $email, $hashpass, $otp, $emailId);
                    ...
                    <form action="" method="POST">
                        <div class="container d-flex justify-content-center align-items-center">
                            <div class="profile-pic">
                                <img alt="Profile Picture" src="../assets/images/HIVE SVG BRAND.svg"/>
                            </div>   
                        </div>
                        <div class="mb-2 mt-3">
